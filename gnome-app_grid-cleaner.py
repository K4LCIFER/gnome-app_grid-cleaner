...
def main():
    # This is the directory for the Gnome App-Grid application files. By
    # default, the directory is defined as `/usr/share/applications`; however,
    # it can be modified to point to any directory.
    DEFAULT_APPLICATION_DIRECTORY = '/usr/share/applications'

    parser = argparse.ArgumentParser()
    # TODO: Perhaps look into verbosity level with verbose being not listed
    # being equal to the minimum level of verbosity (ie quiet which is
    # default).
    parser.add_argument(
        '--verbose',
        '-v',
    ...
    if args.subcommand == 'whitelist':  # For modifying the whitelist.
        if args.add:  # For adding entries to the whitelist.
            with open('whitelist', 'r+') as whitelist:
                # 'r+' is for read and write, but the file pointer is placed at
                # the beginning of the file. The following `read()` command
                # will then put the file pointer at the end of the file.
                whitelist_content = (
                    whitelist.read()
                )  # Store whitelist contents.
                # Loop thuough all of the files that were specified in the
                # command.
                for application_filename in args.add:
                    # If the application filename is already within the
                    # whitelist, then don't add it.
                    if application_filename in whitelist_content:
                        print(application_filename + " is already whitelisted.")
                    # If the application filename is not within the whitelist,
                    # then add it.
                ...
                if args.verbose:
                    for application_filename in args.remove:
                        if (
                            ...
                            not in original_whitelist_contents
                        ):
                            print(application_filename + " is not whitelisted.")
                with open('new_whitelist', 'w') as new_whitelist:
                    # Go through the whitelist's contents, and only keep that
                    # which has not been requested to be removed.
            ...
            with open('whitelist', 'r') as whitelist:
                for line in whitelist:
                    print(line.strip("\n"))
                    # NOTE: The newline is stripped off of each entry so that
                    # there isn't an extra newline character. The other newline
                    # character is generated by the print command.
    ...
    # Apply the whitelist to clean the Gnome App-Grid.
        ...
        for application_filename in os.listdir(DEFAULT_APPLICATION_DIRECTORY):
            ...
            if '.desktop' not in application_filename:
                ...
                # In case the file already contains a line matching
                # `NoDisplay=`, then remove it so that there aren't duplicates.
                ...
                with open(
                    ...
                    with open(
                        ...
                        + application_filename,
                        'w',
                    ) as new_application_file:
                        for line in application_file:
                            # Write the original application file line to the
                            # new application file if it does not match
                            # `NoDisplay`
                            if not nodisplay_match.search(line):
                                new_application_file.write(line)
                        # After we are sure that `NoDisplay` is no longer
                        # present, rewrite it to the file as true to hide the
                        # application.
